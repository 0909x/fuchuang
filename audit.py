...
class ModelAuditor:
    ...
    def _calculate_composite_score(self, success_rate: float) -> float:
        """计算综合评分"""
        cpu_score = (100 - self._get_resource_metrics()['max_cpu']) * 0.3
        memory_score = (100 - self._get_resource_metrics()['max_memory']) * 0.2
        return round(success_rate * 100 * 0.5 + cpu_score + memory_score, 2)
