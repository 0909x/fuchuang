...
async def process_qa_generation(config=None):
    ...
    while current_count < target_count:
        ...
        if new_pairs:
            ...
            # 修改计算成功率的方式：当前批次成功数 / 批次预计生成数
            batch_expected = current_batch_size * qa_per_request
            batch_success_rate = len(new_pairs) / batch_expected if batch_expected > 0 else 0

            logger.info(
                f"已保存 {len(new_pairs)} 题，总计: {total_saved}/{target_count} "
                f"(当前批次成功率: {batch_success_rate:.1%})"  # 修改日志显示
            )
