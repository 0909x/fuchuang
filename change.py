...
返回包含'question'和'type'字段的完整JSON，并确保包含'json'字样"""
    ...
    async def transform(self, question: Dict, operation: str, **params) -> Dict:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.warning(
                    f"变形失败(尝试 {attempt + 1}/{self.max_retries}): {str(e)} "
                    f"(操作: {operation}, 题目ID: {question.get('id', '未知')})"
                )
                await asyncio.sleep(wait_time)
                continue

        logger.error(
            f"达到最大重试次数，放弃变形操作: {operation} "
    ...
    def _build_prompt(self, question: Dict, operation: str, params: Dict) -> str:
        ...
        base_params = {
            ...
            "current_type": question["source_type"],
            "source_lang": "zh" if operation == "translate" and params.get("target_lang") == "en" else "en"
        }

        # 处理特定操作的参数
        if operation == "restructure":
            params["source_type"] = question["source_type"]
            params["target_type"] = params.get("target_type", "unknown")
            conversion_key = f"{question['source_type']}->{params['target_type']}"
        ...
        # 构建提示词
        prompt_template = self.prompt_templates[operation]
        return prompt_template.format(**base_params, **params)
    ...
    async def _process_question(self, question: Question, op: Dict, semaphore: asyncio.Semaphore) -> Optional[Question]:
        """处理单个题目的变形"""
        async with semaphore:
            try:
                # 验证输入数据
                if not isinstance(question.content, dict):
                    raise ValueError("题目内容必须是字典")

                if not op.get("type"):
                    raise ValueError("未指定变形操作类型")

                # 执行变形
                transformed_content = await self.engine.transform(
                ...
                if not transformed_content:
                    raise ValueError("变形结果为空")

                # 创建新版本
                return self._create_new_version(question, transformed_content, op)

            except Exception as e:
                logger.error(
                    f"处理题目 {question.id[:8]} 失败: {str(e)} "
                    f"(操作: {op.get('type', '未知')})"
                )
                return None
    ...
    def _parse_response(self, response: str, operation: str) -> Dict:
        ...
        try:
            ...
            raise ValueError("API返回了无效的JSON格式") from e
        except Exception as e:
            logger.error(f"响应解析失败: {str(e)}")
            raise ValueError("无法解析API响应") from e
